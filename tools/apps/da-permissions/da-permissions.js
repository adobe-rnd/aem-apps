...
class DaPermissionsApp extends LitElement {
  static properties = {
    ...
    _grantWho: { state: true },
    _grantLevel: { state: true },
    _grantSites: { state: true },
    _grantSiteSearch: { state: true },
  };
  ...
  connectedCallback() {
    ...
    this._grantWho = '';
    this._grantLevel = 'write';
    this._grantSites = new Set();
    this._grantSiteSearch = '';
  ...
  // ---- Grant site access form ----

  async handleGrantSubmit() {
    const who = this._grantWho.trim();
    if (!who || this._grantSites.size === 0 || this._isProcessing) return;
    ...
    const actions = levelToActions(this._grantLevel);
    [...this._grantSites].forEach((site) => {
      const path = `/${site}/ + **`;
      const existing = config.permissions.data.find(
        (r) => normPath(r.path) === normPath(path) && r.actions === actions,
      );
      if (existing) {
        const current = splitGroups(existing.groups);
        if (!current.includes(who)) existing.groups = [...current, who].join(', ');
      } else {
        config.permissions.data.push({ path, groups: who, actions });
      }
    });
    ...
    const grantedSites = [...this._grantSites];
    const grantedLevel = this._grantLevel;
    const result = await updateOrgConfig(this._org, config);
    if (result.success) {
      this._rules = config.permissions.data;
      this._grantWho = '';
      ...
      this._message = {
        type: 'success',
        text: `Granted ${grantedLevel} access to: ${grantedSites.join(', ')}.`,
      };
    } else if (result.status === 403 || result.status === 401) {
      this._readOnly = true;
    } else {
  ...
  // ---- Render: org-level Content Permissions slot (/ + **) with site grant ----

  renderOrgContentSlot(slot, sectionTitle) {
    const label = pathLabel(slot.path, this._site);
    const cls = pathClass(slot.path, this._site);
    const labelEl = cls === 'scope-folder'
      ? html`<span class="perm-slot-label ${cls}">${label}</span>`
    ...
    const q = this._grantSiteSearch.toLowerCase();
    const filteredSites = q
      ? this.allSites.filter((s) => s.toLowerCase().includes(q))
      : this.allSites;
    const allChecked = filteredSites.length > 0
      && filteredSites.every((s) => this._grantSites.has(s));
    const canGrant = this._grantWho.trim() && this._grantSites.size > 0 && !this._isProcessing;
    ...
    return html`
      ...
      <div class="perm-slot">
        ...
        ${this.renderLevelRow(slot, 'write')}
        ${slot.none.length > 0 ? this.renderLevelRow(slot, 'none') : nothing}
        <div class="grant-sites-section">
          <div class="grant-sites-header">
            <span class="grant-sites-title">Specific sites</span>
            <input
              class="grant-sites-search"
              type="search"
              placeholder="Search sites…"
              .value=${this._grantSiteSearch}
              @input=${(e) => { this._grantSiteSearch = e.target.value; }}
            />
          </div>
          <div class="grant-sites-list">
            ${this.allSites.length === 0 ? html`
              <span class="grant-sites-empty">No sites in this org yet.</span>
            ` : html`
              <label class="grant-site-item grant-site-select-all">
                <input type="checkbox" .checked=${allChecked}
                  @change=${(e) => { this._grantSites = e.target.checked ? new Set(filteredSites) : new Set(); }} />
                <span>Select all</span>
              </label>
              ${filteredSites.map((site) => html`
                <label class="grant-site-item">
                  <input type="checkbox" .checked=${this._grantSites.has(site)}
                    @change=${(e) => { const next = new Set(this._grantSites); if (e.target.checked) next.add(site); else next.delete(site); this._grantSites = next; }} />
                  <span>${site}</span>
                </label>
              `)}
            `}
          </div>
          <div class="grant-sites-form">
            <input
              class="grant-who-input"
              type="text"
              placeholder="email or IMS group ID"
              .value=${this._grantWho}
              @input=${(e) => { this._grantWho = e.target.value; }}
            />
            <div class="grant-level-toggle">
              <button
                class="grant-level-btn ${this._grantLevel === 'read' ? 'is-active grant-level-read' : ''}"
                @click=${() => { this._grantLevel = 'read'; }}
              >Read</button>
              <button
                class="grant-level-btn ${this._grantLevel === 'write' ? 'is-active grant-level-write' : ''}"
                @click=${() => { this._grantLevel = 'write'; }}
              >Write</button>
            </div>
            <sl-button class="pw-fill-accent" ?disabled=${!canGrant} @click=${this.handleGrantSubmit}>
              Grant
            </sl-button>
          </div>
        </div>
  ...
  renderAdmin() {
    ...
    const renderSlotEntry = (slot) => {
      const title = orgSectionTitles[slot.path] ?? null;
      if (!this._site && slot.path === '/ + **') return this.renderOrgContentSlot(slot, title);
      return this.renderSlot(slot, title);
    };
    return html`
      ${this._readOnly ? html`
      ...
      <div class="slots-list">
        ${this.permissionSlots.map(renderSlotEntry)}
