...
/**
 * Content Replace - Advanced Search & Replace Tool for DA Platform
...
import {
  ...
  createSearchRegex as createSearchRegexCore,
  findHtmlMatches,
  formatHTML as formatHTMLCore,
  getMatchContext as getMatchContextCore,
...
async function createVersion(path, description = 'Version created by Content Replace') {
  const { token } = app;
  const cleanPath = path.startsWith('/') ? path.substring(1) : path;
...
async function revertToPreContentSweepVersion(filePath) {
  try {
...
async function bulkRevertLastReplacement() {
  ...
  try {
    ...
    const revertPromises = selectedResults.map(async (result, index) => {
      ...
      const success = await revertToPreContentSweepVersion(filePath);
...
function getMatchContext(content, index, contextLength = 75) {
  return getMatchContextCore(content, index, contextLength);
}
...
function createSearchRegex(searchTerm, searchType, caseSensitive) {
  return createSearchRegexCore(searchTerm, searchType, caseSensitive);
}
...
function searchAndReplaceHTML(content, searchTerm, replaceTerm = '', caseSensitive = false) {
  ...
  const processedSearchTerm = buildFlexibleHtmlSearchPattern(searchTerm, searchType);
  const matches = findHtmlMatches(searchContent, searchTerm, searchType, caseSensitive);
...
function toggleAccordion(contentId) {
  ...
  if (content && card) {
    ...
    if (isExpanded) {
      ...
      if (accordionIcon) {
        accordionIcon.src = '/tools/apps/content-replace/icons/chevron-down.svg';
      }
    } else {
      // Expand
      ...
      if (accordionIcon) {
        accordionIcon.src = '/tools/apps/content-replace/icons/chevron-up.svg';
      }
    }
  }
}
